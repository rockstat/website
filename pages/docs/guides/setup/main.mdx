...
import { TerminalInput } from 'app/components/text/terminal'
import Image from 'app/components/image'

export const meta = {
  title: 'Установка и запуск Rockstat',
}


# Установка и запуск Rockstat
...
проверим, что установлен curl, который требуется для загрузки файлов из интернета.

```bash
sudo apt -y update && sudo apt -y install curl
```

Для запуска скрипта установки/обновления системы выполните в командной строке
...
<img src="/media/casts/prepare3.svg" className="asciicast" />
...
<img src="/media/casts/setup1.svg" className="asciicast" />
...
```
[[tip]]
|**Получение к ключей VPN с сервера**
...
специальный ключ `public_keys/support.pub`, при помощи которого сервер обновлений может подключатся к серверу Rockstat
