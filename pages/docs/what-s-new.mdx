...
- **Band**
  - Band Set updated to latest Band API
...
- **Infrastructure**
  - Вывод логов сервисом обновления SSL сертификата
  - new Theia with paramaneters interface and many other improvements
  - new Anaconda with py3.7 and latest ipython + ipykernell with async support
  - Public directory to store world visible files. Accessible in Theia
  - Ability to manage CHWriter and Front custom configuration with Theia
