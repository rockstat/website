/**
 * generated automatically / don't change manually!
 */

export default [
  {
    "name": "Введение",
    "items": [
      {
        "name": "О платформе",
        "href": "/docs",
        "path": "0-0",
        "enabled": true,
        "modified": "2018-12-18"
      },
      {
        "name": "Быстрый старт",
        "href": "/docs/intro/getting-started",
        "path": "0-1",
        "enabled": true,
        "modified": "2019-01-31"
      },
      {
        "name": "Что нового",
        "href": "/docs/what-s-new",
        "path": "0-2",
        "enabled": true,
        "modified": "2019-01-31"
      }
    ],
    "path": "0",
    "enabled": true
  },
  {
    "name": "Архитектура",
    "items": [
      {
        "name": "Основные компоненты",
        "href": "/docs/architecture/overview",
        "path": "1-0",
        "enabled": true,
        "modified": "2019-01-31"
      },
      {
        "name": "Обработка входящих запросов",
        "items": [
          {
            "name": "Жизненный цикл и адресация",
            "href": "/docs/architecture/request-lifecycle",
            "path": "1-1-0",
            "enabled": true,
            "modified": "2019-01-31"
          },
          {
            "name": "Коммуникация сервисов",
            "href": "/docs/architecture/services-commnication",
            "path": "1-1-1"
          }
        ],
        "path": "1-1",
        "hrefs": [
          "/docs/architecture/request-lifecycle",
          "/docs/architecture/services-commnication"
        ],
        "enabled": true
      }
    ],
    "path": "1",
    "enabled": true
  },
  {
    "name": "Руководство пользователя",
    "items": [
      {
        "name": "Установка платформы",
        "items": [
          {
            "name": "Выбор, подготовка сервера и DNS",
            "href": "/docs/guides/setup/server_and_domain",
            "path": "2-0-0",
            "enabled": true,
            "modified": "2019-01-31"
          },
          {
            "name": "Запуск инсталлятора",
            "href": "/docs/guides/setup/installation",
            "path": "2-0-1",
            "enabled": true,
            "modified": "2019-02-08"
          }
        ],
        "path": "2-0",
        "hrefs": [
          "/docs/guides/setup/server_and_domain",
          "/docs/guides/setup/installation"
        ],
        "enabled": true
      },
      {
        "name": "События на сайте - Web-SDK",
        "href": "/docs/guides/web-sdk/setup",
        "path": "2-1",
        "enabled": true,
        "modified": "2019-03-14"
      },
      {
        "name": "Использование пикселя 1x1.gif",
        "href": "/docs/guides/collecting/pixel",
        "path": "2-2",
        "enabled": true,
        "modified": "2019-01-31"
      },
      {
        "name": "Получение данных из сервисов",
        "items": [
          {
            "name": "Yandex AppMetrika",
            "href": "/docs/guides/collecting/appmetrika",
            "path": "2-3-0",
            "enabled": true,
            "modified": "2019-01-31"
          }
        ],
        "path": "2-3",
        "hrefs": [
          "/docs/guides/collecting/appmetrika"
        ],
        "enabled": true
      },
      {
        "name": "Использование Панели управления",
        "href": "/docs/guides/dashboard/using",
        "path": "2-4"
      },
      {
        "name": "Создание сервисов",
        "href": "/docs/guides/services/create-service-using-skeleton",
        "path": "2-5",
        "enabled": true,
        "modified": "2019-01-31"
      },
      {
        "name": "Использование инструментов из комплекта",
        "items": [
          {
            "name": "Работа с Theia",
            "href": "/docs/guides/theia/using",
            "path": "2-6-0"
          },
          {
            "name": "Мониторинг Netdata",
            "href": "/docs/guides/netdata/using",
            "path": "2-6-1"
          },
          {
            "name": "Работа с Jupyter",
            "href": "/docs/guides/jupyter/using",
            "path": "2-6-2"
          }
        ],
        "path": "2-6",
        "hrefs": [
          "/docs/guides/theia/using",
          "/docs/guides/netdata/using",
          "/docs/guides/jupyter/using"
        ]
      },
      {
        "name": "Работа с ClickHouse",
        "items": [
          {
            "name": "Основные принципы",
            "href": "/docs/guides/clickhouse/base",
            "path": "2-7-0"
          },
          {
            "name": "Модификация схемы",
            "href": "/docs/guides/clickhouse/schema-modify",
            "path": "2-7-1"
          }
        ],
        "path": "2-7",
        "hrefs": [
          "/docs/guides/clickhouse/base",
          "/docs/guides/clickhouse/schema-modify"
        ]
      },
      {
        "name": "Подключение к VPN",
        "href": "/docs/guides/connect-vpn",
        "path": "2-8"
      },
      {
        "name": "Отладка",
        "items": [
          {
            "name": "Отладка HTTP запров",
            "href": "/docs/guides/debug/http-debug-service",
            "path": "2-9-0",
            "enabled": true,
            "modified": "2019-01-31"
          }
        ],
        "path": "2-9",
        "hrefs": [
          "/docs/guides/debug/http-debug-service"
        ],
        "enabled": true
      },
      {
        "name": "Хитрости конфигурации",
        "href": "/docs/guides/configuration-tricks",
        "path": "2-10",
        "enabled": true,
        "modified": "2019-01-31"
      }
    ],
    "path": "2",
    "enabled": true
  },
  {
    "name": "Справочная информация",
    "items": [
      {
        "name": "HTTP Redirect",
        "href": "/docs/reference/collecting/redirect",
        "path": "3-0"
      },
      {
        "name": "HTTP Webhook/Postback",
        "href": "/docs/reference/collecting/webhooks",
        "path": "3-1",
        "enabled": true,
        "modified": "2019-01-31"
      },
      {
        "name": "Каналы получения данных",
        "items": [
          {
            "name": "WebSocket",
            "href": "/docs/reference/collecting/websocket",
            "path": "3-2-0"
          },
          {
            "name": "Загрузка больших файлов",
            "href": "/docs/reference/collecting/large-files",
            "path": "3-2-1"
          }
        ],
        "path": "3-2",
        "hrefs": [
          "/docs/reference/collecting/websocket",
          "/docs/reference/collecting/large-files"
        ]
      },
      {
        "name": "Web-SDK",
        "items": [
          {
            "name": "Сниппет и параметры конфигурации",
            "href": "/docs/reference/web-sdk/snippet-and-configuration",
            "path": "3-3-0",
            "enabled": true,
            "modified": "2019-01-31"
          },
          {
            "name": "Базовая структура событий",
            "href": "/docs/reference/web-sdk/event-struct",
            "path": "3-3-1",
            "enabled": true,
            "modified": "2019-01-31"
          },
          {
            "name": "Автоматичексие события",
            "href": "/docs/reference/web-sdk/auto-track",
            "path": "3-3-2",
            "enabled": true,
            "modified": "2019-01-31"
          },
          {
            "name": "Процессинг сессий",
            "href": "/docs/reference/web-sdk/sessions",
            "path": "3-3-3",
            "enabled": true,
            "modified": "2019-01-31"
          },
          {
            "name": "JavaScript API",
            "href": "/docs/reference/web-sdk/js-api",
            "path": "3-3-4",
            "enabled": true,
            "modified": "2019-01-31"
          },
          {
            "name": "Перехват пикселей",
            "href": "/docs/reference/web-sdk/catch-pixel",
            "path": "3-3-5",
            "enabled": true,
            "modified": "2019-01-31"
          },
          {
            "name": "Используемый транспорт",
            "href": "/docs/reference/web-sdk/transport",
            "path": "3-3-6"
          }
        ],
        "path": "3-3",
        "hrefs": [
          "/docs/reference/web-sdk/snippet-and-configuration",
          "/docs/reference/web-sdk/event-struct",
          "/docs/reference/web-sdk/auto-track",
          "/docs/reference/web-sdk/sessions",
          "/docs/reference/web-sdk/js-api",
          "/docs/reference/web-sdk/catch-pixel",
          "/docs/reference/web-sdk/transport"
        ],
        "enabled": true
      },
      {
        "name": "JSON-RPC 2.0 RST",
        "href": "/docs/reference/json-rpc-rst",
        "path": "3-4"
      },
      {
        "name": "API сервисов",
        "items": [
          {
            "name": "Director API",
            "href": "/docs/reference/director-service/api",
            "path": "3-5-0"
          },
          {
            "name": "Front API",
            "href": "/docs/reference/font-service/api",
            "path": "3-5-1"
          }
        ],
        "path": "3-5",
        "hrefs": [
          "/docs/reference/director-service/api",
          "/docs/reference/font-service/api"
        ]
      },
      {
        "name": "Band Framework (Python)",
        "items": [
          {
            "name": "Слекет сервиса",
            "href": "/docs/reference/band-framework/skeleton",
            "path": "3-6-0",
            "enabled": true,
            "modified": "2019-01-31"
          },
          {
            "name": "Программное API",
            "href": "/docs/reference/band-framework/python-api",
            "path": "3-6-1",
            "enabled": true,
            "modified": "2019-01-31"
          },
          {
            "name": "Файл конфигурации",
            "href": "/docs/reference/band-framework/configs",
            "path": "3-6-2",
            "enabled": true,
            "modified": "2019-01-31"
          }
        ],
        "path": "3-6",
        "hrefs": [
          "/docs/reference/band-framework/skeleton",
          "/docs/reference/band-framework/python-api",
          "/docs/reference/band-framework/configs"
        ],
        "enabled": true
      },
      {
        "name": "Simple ClickHouse (Python)",
        "href": "/docs/reference/simplech",
        "path": "3-7",
        "enabled": true,
        "modified": "2019-01-31"
      },
      {
        "name": "RockMe Framework (TypeScript)",
        "href": "/docs/reference/rockme-framework/api",
        "path": "3-8"
      },
      {
        "name": "Переменные окружения",
        "href": "/docs/reference/environment",
        "path": "3-9"
      },
      {
        "name": "Маппинг путей",
        "href": "/docs/reference/directories",
        "path": "3-10"
      },
      {
        "name": "Сетевая инфраструктура",
        "href": "/docs/reference/network",
        "path": "3-11"
      },
      {
        "name": "Схема хранилища ClickHouse",
        "href": "/docs/reference/clickhouse/schema",
        "path": "3-12"
      }
    ],
    "path": "3",
    "enabled": true
  },
  {
    "name": "Создание сервисов",
    "items": [
      {
        "name": "Python + Band Framework",
        "items": [
          {
            "name": "Организация сервиса",
            "href": "/docs/build-service-band/stucture",
            "path": "4-0-0"
          },
          {
            "name": "Коммуникация с другими сервисами",
            "href": "/docs/build-service-band/call-other-method",
            "path": "4-0-1"
          },
          {
            "name": "Работа с ClickHouse",
            "href": "/docs/build-service-band/clickhouse",
            "path": "4-0-2"
          },
          {
            "name": "Другие возможности",
            "href": "/docs/build-service-band/other-functions",
            "path": "4-0-3"
          }
        ],
        "path": "4-0",
        "hrefs": [
          "/docs/build-service-band/stucture",
          "/docs/build-service-band/call-other-method",
          "/docs/build-service-band/clickhouse",
          "/docs/build-service-band/other-functions"
        ]
      },
      {
        "name": "TypeScript + Rockme Framework",
        "items": [
          {
            "name": "Организация сервиса",
            "href": "/docs/build-service-rockme/stucture",
            "path": "4-1-0"
          }
        ],
        "path": "4-1",
        "hrefs": [
          "/docs/build-service-rockme/stucture"
        ]
      }
    ],
    "path": "4"
  },
  {
    "name": "Туториалы",
    "items": [
      {
        "name": "Установка и настройка Metabase",
        "href": "/docs/tutorials/metabase",
        "path": "5-0",
        "enabled": true,
        "modified": "2019-04-10"
      },
      {
        "name": "Получение данных из Google Adwords",
        "href": "/docs/tutorials/sources/adwords",
        "path": "5-1",
        "enabled": true,
        "modified": "2019-01-31"
      },
      {
        "name": "Получение данных из других сервисов",
        "items": [
          {
            "name": "Yandex AppMetrika",
            "href": "/docs/tutorials/sources/appmetrika",
            "path": "5-2-0",
            "enabled": true,
            "modified": "2019-01-31"
          }
        ],
        "path": "5-2",
        "hrefs": [
          "/docs/tutorials/sources/appmetrika"
        ],
        "enabled": true
      },
      {
        "name": "Сбор сырых данных Google Analytics",
        "href": "/docs/tutorials/google-analytics-raw-data",
        "path": "5-3"
      },
      {
        "name": "Создание динамического Calltracking",
        "href": "/docs/tutorials/dynamic-calltracking",
        "path": "5-4"
      },
      {
        "name": "Собственный формат событий",
        "href": "/docs/tutorials/custom-event-table",
        "path": "5-5",
        "enabled": true,
        "modified": "2019-01-31"
      },
      {
        "name": "Построение истории отдельного пользователя",
        "href": "/docs/tutorials/user-history",
        "path": "5-6"
      },
      {
        "name": "Классические модели атрибуции",
        "href": "/docs/tutorials/attribution/classic",
        "path": "5-7"
      },
      {
        "name": "Вероятностное прохождение воронки",
        "href": "/docs/tutorials/attribution/funnel",
        "path": "5-8"
      },
      {
        "name": "Атрибуция по индексу активности",
        "href": "/docs/tutorials/attribution/activity",
        "path": "5-9"
      },
      {
        "name": "Воронки",
        "href": "/docs/tutorials/funnel",
        "path": "5-10"
      },
      {
        "name": "Реализация Cookie-Sync",
        "href": "/docs/tutorials/cookie-sync",
        "path": "5-11"
      },
      {
        "name": "Сегментация пользователей",
        "href": "/docs/tutorials/segmentation",
        "path": "5-12"
      }
    ],
    "path": "5",
    "enabled": true
  }
]